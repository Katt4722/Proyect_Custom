...
user_histories = {}  # Acá se guarda el historial 
...
# Cargar datos 
    ...
    {json.dumps(bot.fashion_data, ensure_ascii=False, indent=2)}
    ...
    def handle_text(message: tlb.types.Message):
        ...
        response = bot.get_groq_fashion_response(message.text)
        if response:
    ...
    def handle_voice(message: tlb.types.Message):
        ...
        # Transcribir audio
        transcription = bot.transcribe_voice_with_groq(message)
        ...
        response = bot.get_groq_fashion_response(" ".join([m["content"] for m in historial if m["role"]=="user"]))
